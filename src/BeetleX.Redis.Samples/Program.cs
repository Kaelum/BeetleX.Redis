...
namespace BeetleX.Redis.Samples
{
    class Program
    {
        static void Main(string[] args)
        {
			...
			Subscriber subscribe = Redis.Subscribe();
        ...
        static async void Test()
        {
			...
			(Employee, Employee, Employee) items = await Redis.Get<Employee, Employee, Employee>("key1", "key2", "key3");
            Write(items.Item1);
            Write(items.Item2);
            Write(items.Item3);
            Line();
			...
			RedisList<Employee> list = Redis.CreateList<Employee>("employees");
			...
			//  await Redis.Default.Flushall();
			RedisHashTable table = Redis.CreateHashTable("myhash");
