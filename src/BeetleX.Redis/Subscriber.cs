...
namespace BeetleX.Redis
{
    public class Subscriber : IDisposable
    {
        ...
        private async void OnReadMessave()
        {
            ...
            while (true)
            {
				Result result = await subscribeRequest.Read();
                if (result.ResultType == ResultType.NetError || result.ResultType == ResultType.DataError || result.ResultType == ResultType.Error)
                {
                    ...
                    {
						PipeStream pipeStream = redisClient.TcpClient.Stream.ToPipeStream();
        ...
        {
			System.Collections.Generic.List<ResultItem> items = result.Data;
        ...
        private bool OnBlockingRead(Result r, PipeStream stream, RedisClient c)
        {
            ...
            if (index == 0 || index == 1)
            {
				...
				ResultItem item = new ResultItem { Type = ResultType.String, Data = stream.ReadString(r.BodyLength.Value) };
                r.Data.Add(item);
                return false;
            }
            else
            {
                ...
                else
                {
                    try
                    {
						ResultItem item = new ResultItem { Type = ResultType.Object, Data = mDB.DataFormater.DeserializeObject(register.MessageType, c, stream, r.BodyLength.Value) };
                        r.Data.Add(item);
                    }
                    catch (Exception e_)
                    {
