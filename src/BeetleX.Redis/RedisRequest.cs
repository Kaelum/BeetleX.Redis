            ...
            else
            {
                if (Result.BodyLength == null)
                {
                    if (pipeStream.TryReadLine(out string line))
                    {
                        if (line[0] == ':')
                        {
							ResultItem item = new ResultItem { Type = ResultType.Integers, Data = long.Parse(line.Substring(1, line.Length - 1)) };
                        ...
                        {
							ResultItem item = new ResultItem { Type = ResultType.Simple, Data = line.Substring(1, line.Length - 1) };
                            Result.Data.Add(item);
                            Result.ArrayReadCount++;
                            Result.ReadCount++;
                        }
                        ...
                        {
							ResultItem item = new ResultItem { Type = ResultType.Error, Data = line.Substring(1, line.Length - 1) };
                            Result.Data.Add(item);
                            Result.ArrayReadCount++;
                            Result.ReadCount++;
                        }
                ...
                if (Result.BodyLength != null)
                {
                    if ((Result.BodyLength == -1))
                    {
                        ...
                        {
							ResultItem item = new ResultItem { Type = ResultType.NotFound, Data = null };
                            Result.Data.Add(item);
                        }
