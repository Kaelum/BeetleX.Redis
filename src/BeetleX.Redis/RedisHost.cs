...
namespace BeetleX.Redis
{
    public class RedisHost : IDisposable
    {
        ...
        {
			RedisClient client = new RedisClient(SSL, Host, Port);
			Result result = Connect(client);
            if (result.IsError)
            {
            ...
            if (!client.TcpClient.IsConnected)
            {
                ...
                if (client.TcpClient.Connect(out isNew))
                {
                    ...
                    if (!string.IsNullOrEmpty(Password))
                    {
						...
						Task<Result> task = request.Execute();
                        task.Wait();
        ...
        public async void Ping()
        {
            if (System.Threading.Interlocked.CompareExchange(ref mPingStatus, 1, 0) == 0)
            {
                ...
                try
                {
					...
					RedisRequest request = new RedisRequest(null, mPingClient, ping, typeof(string));
					Result result = await request.Execute();
                }
                catch
                {
